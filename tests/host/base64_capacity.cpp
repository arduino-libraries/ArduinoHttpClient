...
int main() {
  ...
  for (int i = 0; i < 7; ++i) {
    ...
    for (int capacity = 0; capacity < length; ++capacity) {
      ...
      assert(b64_encode((const unsigned char*)inputs[i], strlen(inputs[i]), output, capacity) == length);
      for (unsigned char byte : output) assert(byte == 0x5a);
    }
    unsigned char output[16];
    memset(output, 0x5a, sizeof output);
    assert(b64_encode((const unsigned char*)inputs[i], strlen(inputs[i]), output, length) == length);
